...
const SERVICES = [
  ...
  { icon: '🧭', name: 'IT Consulting', desc: 'Tech stack decisions, cloud cost reviews, architecture audits. Honest advice, no vendor lock-in.', color: '#F43F5E', bg: '#FFF1F2' },
]
...
const HIGHLIGHTS = [
  { icon: '🏥', title: 'Smart Check-In', desc: 'Walk-in patients scan IC and get a queue ticket in under 60 seconds.', color: '#4F46E5', bg: '#EEF2FF' },
  ...
  { icon: '📡', title: 'Works Offline', desc: 'System stays fully operational even without internet.', color: '#06B6D4', bg: '#ECFEFF' },
  ...
  { icon: '☁️', title: 'Cloud Backup', desc: 'Patient data syncs to cloud automatically — nothing lost.', color: '#10B981', bg: '#ECFDF5' },
]
const WHY = [
...
function MeshBg() {
  return (
    <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', pointerEvents: 'none' }}>
      <div style={{ position: 'absolute', top: '-20%', right: '-10%', width: 'clamp(300px,50vw,700px)', height: 'clamp(300px,50vw,700px)', borderRadius: '50%', background: 'radial-gradient(circle, rgba(99,102,241,0.12) 0%, transparent 70%)' }} />
      <div style={{ position: 'absolute', top: '20%', left: '-15%', width: 'clamp(200px,35vw,500px)', height: 'clamp(200px,35vw,500px)', borderRadius: '50%', background: 'radial-gradient(circle, rgba(6,182,212,0.10) 0%, transparent 70%)' }} />
...
function Reveal({ children, delay = 0, style = {} }) {
  ...
  return <div ref={ref} style={{ opacity: 0, transform: 'translateY(24px)', transition: `opacity .65s ease ${delay}s, transform .65s ease ${delay}s`, ...style }}>{children}</div>
}
    ...
    <div style={{ background: '#fff', border: '1px solid #E4E8FF', borderRadius: 16, overflow: 'hidden', boxShadow: '0 24px 60px rgba(79,70,229,0.12)' }}>
      ...
      <div style={{ padding: 12 }}>
        ...
        <div style={{ background: 'linear-gradient(135deg,#FFF7ED,#FFFBEB)', border: '1px solid #FED7AA', borderRadius: 8, padding: '8px 10px' }}>
          <div style={{ fontSize: 7, color: '#D97706', fontWeight: 700, letterSpacing: '.08em', textTransform: 'uppercase', marginBottom: 4 }}>⚡ AI Triage — #03</div>
      ...
      <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 100, padding: '14px 0', background: scrolled ? 'rgba(255,255,255,0.95)' : 'transparent', backdropFilter: scrolled ? 'blur(20px)' : 'none', borderBottom: scrolled ? '1px solid #E4E8FF' : '1px solid transparent', transition: 'all .3s' }}>
        <div style={{ maxWidth: 1160, margin: '0 auto', padding: '0 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 28 }} className="hide-mobile">
            ...
            <a href="mailto:hello@harnova.my" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '9px 20px', background: 'linear-gradient(135deg,#4F46E5,#6366F1)', borderRadius: 10, fontSize: '.875rem', fontWeight: 600, color: '#fff', boxShadow: '0 4px 12px rgba(79,70,229,0.3)' }}>
              Get in touch →
            </a>
          </div>
          {/* Mobile hamburger */}
          <button className="show-mobile" onClick={() => setMobileOpen(!mobileOpen)}
            style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 8, color: '#0F172A', fontSize: '1.4rem', lineHeight: 1 }}>
            {mobileOpen ? '✕' : '☰'}
          </button>
        </div>
        {/* Mobile menu */}
...
export default function App() {
  ...
  const btnP = { display:'inline-flex',alignItems:'center',gap:8,padding:'13px 26px',background:'linear-gradient(135deg,#4F46E5,#6366F1)',borderRadius:12,fontWeight:700,fontSize:'.95rem',color:'#fff',boxShadow:'0 4px 18px rgba(79,70,229,0.35)',transition:'all .2s' }
  const btnS = { display:'inline-flex',alignItems:'center',gap:8,padding:'13px 26px',border:'2px solid #E4E8FF',borderRadius:12,fontWeight:600,fontSize:'.95rem',color:'#475569',background:'#fff',transition:'all .2s' }
  ...
  return <>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%,420px), 1fr))', gap: 'clamp(32px,5vw,64px)', alignItems: 'center' }}>
          <Reveal>
            ...
            <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 48 }}>
              <a href="#products" style={btnP}>See our products →</a>
              <a href="mailto:hello@harnova.my" style={btnS}>Talk to us</a>
            </div>
            <div style={{ display: 'flex', gap: 'clamp(24px,4vw,40px)', flexWrap: 'wrap', paddingTop: 32, borderTop: '1px solid #E4E8FF' }}>
              {[['99%','Uptime SLA'],['6+','Solutions shipped'],['🇲🇾','Malaysia-first']].map(([n,l]) => (
                <div key={l}>
                  <div style={{ fontFamily:'Syne,sans-serif', fontSize:'clamp(1.3rem,3vw,1.6rem)', fontWeight:800, color:'#4F46E5', lineHeight:1 }}>{n}</div>
          ...
          <Reveal delay={0.1}>
            <div style={{ position: 'relative', height: 'clamp(360px,50vw,520px)', display: 'flex', flexDirection: 'column', gap: 12 }}>
              {/* Main MediLink card */}
              <div style={{ background:'#fff', border:'1px solid #E4E8FF', borderRadius:16, padding:18, boxShadow:'0 8px 32px rgba(79,70,229,0.10)' }}>
                <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:14 }}>
                  <div style={{ width:34, height:34, borderRadius:10, background:'linear-gradient(135deg,#4F46E5,#8B5CF6)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'.9rem' }}>🏥</div>
                  <div>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
                <div style={{ background:'#fff', border:'1px solid #FED7AA', borderRadius:14, padding:14 }}>
                  <div style={{ fontSize:'.65rem', color:'#D97706', fontWeight:700, textTransform:'uppercase', letterSpacing:'.07em', marginBottom:6 }}>⚡ AI Triage</div>
                ...
                <div style={{ background:'#fff', border:'1px solid #D1FAE5', borderRadius:14, padding:14 }}>
                  <div style={{ fontSize:'.65rem', color:'#059669', fontWeight:700, textTransform:'uppercase', letterSpacing:'.07em', marginBottom:6 }}>✓ Payment</div>
              ...
              <div style={{ background:'#fff', border:'1px solid #E4E8FF', borderRadius:14, padding:'12px 14px', display:'flex', alignItems:'center', gap:10 }}>
                <div style={{ width:30, height:30, borderRadius:8, background:'#ECFDF5', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'1rem', flexShrink:0 }}>☁️</div>
                <div>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(min(100%,280px),1fr))', gap:16 }}>
            {SERVICES.map(s => (
              <div key={s.name}
                ...
                <div style={{ width:44, height:44, borderRadius:12, background:s.bg, border:`1px solid ${s.color}20`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:'1.3rem', marginBottom:14 }}>{s.icon}</div>
    ...
    {/* ── PRODUCTS ─────────────────────────────────────────────────────── */}
    <section id="products" style={{ padding:'clamp(64px,8vw,100px) 0', background:'linear-gradient(160deg,#F8F9FF,#EEF2FF)' }}>
      <div style={W}>
        <Reveal style={{ textAlign:'center', marginBottom:'clamp(36px,5vw,56px)' }}>
          ...
          <div style={{ background:'#fff', border:'1px solid #E4E8FF', borderRadius:20, padding:'clamp(28px,5vw,56px)', display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(min(100%,340px),1fr))', gap:'clamp(28px,5vw,56px)', alignItems:'center', boxShadow:'0 12px 48px rgba(79,70,229,0.07)' }}>
            <div>
              ...
              <div style={{ display:'flex', gap:12, flexWrap:'wrap' }}>
                <a href="mailto:hello@harnova.my?subject=MediLink Demo" style={btnP}>Request a demo →</a>
                <a href="https://github.com/yashhgill/Medilink" target="_blank" rel="noopener" style={btnS}>GitHub</a>
              </div>
            </div>
            <Mockup />
          </div>
        ...
        {/* Feature highlights */}
        <Reveal delay={.1} style={{ marginBottom:24 }}>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(min(100%,220px),1fr))', gap:12 }}>
            {HIGHLIGHTS.map(h => (
              <div key={h.title}
                ...
                <div style={{ width:34, height:34, borderRadius:9, background:h.bg, display:'flex', alignItems:'center', justifyContent:'center', fontSize:'1rem', flexShrink:0, border:`1px solid ${h.color}20` }}>{h.icon}</div>
                <div>
        ...
        <Reveal delay={.15}>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(min(100%,240px),1fr))', gap:14 }}>
            {COMING.map(p => (
              <div key={p.name} style={{ background:'#fff', border:'2px dashed #E4E8FF', borderRadius:14, padding:'24px 20px', position:'relative', opacity:.8 }}>
                <span style={{ position:'absolute', top:14, right:14, padding:'2px 8px', background:'#FFFBEB', border:'1px solid #FCD34D', borderRadius:5, fontSize:'.62rem', fontWeight:700, color:'#D97706', textTransform:'uppercase', letterSpacing:'.07em' }}>Coming Soon</span>
                ...
                <div style={{ fontSize:'.8rem', color:'#64748B', lineHeight:1.6 }}>{p.desc}</div>
              </div>
            ))}
          </div>
        </Reveal>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:24 }}>
              {WHY.map(w => (
                <div key={w.title} style={{ display:'flex', gap:14 }}>
                  <div style={{ width:42, height:42, borderRadius:11, background:'linear-gradient(135deg,#EEF2FF,#F5F3FF)', border:'1px solid #C7D2FE', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'1.1rem', flexShrink:0 }}>{w.icon}</div>
                  <div>
        ...
        <Reveal>
          <div style={{ background:'#fff', border:'1px solid #E4E8FF', borderRadius:24, padding:'clamp(36px,6vw,72px) clamp(24px,5vw,72px)', textAlign:'center', position:'relative', overflow:'hidden', boxShadow:'0 20px 60px rgba(79,70,229,0.08)' }}>
            ...
            <div style={{ display:'flex', gap:12, justifyContent:'center', flexWrap:'wrap', marginBottom:32 }}>
              <a href="mailto:hello@harnova.my" style={btnP}>Email us →</a>
              <a href="https://wa.me/60182085097" target="_blank" rel="noopener" style={btnS}>💬 WhatsApp</a>
            </div>
